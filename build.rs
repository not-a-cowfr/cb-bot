...
fn process_dir(
	...
	for entry in
		std::fs::read_dir(dir_path).expect(&format!("Failed to read {:?} directory", dir_path))
	{
...
fn main() {
	...
	std::fs::write("src/commands/mod.rs", mod_content).expect("Failed to write to mod.rs");

	for entry in std::fs::read_dir(&base_path).expect("Failed to read src/commands/ directory") {
		if let Ok(entry) = entry {
			let path = entry.path();
			if path.is_dir() {
				...
				std::fs::write(mod_path, mod_content).expect("Failed to write subdirectory mod.rs");
			}
		}
	}
}
